...
CREATE OR REPLACE FUNCTION reject_intake_event_mutation()
RETURNS trigger AS $$
BEGIN
    RAISE EXCEPTION 'intake_events are append-only';
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS intake_events_no_update ON intake_events;
CREATE TRIGGER intake_events_no_update
BEFORE UPDATE OR DELETE ON intake_events
FOR EACH ROW EXECUTE FUNCTION reject_intake_event_mutation();
